...
params: 
  ...
  indfilter: FALSE
  cookscutoff: TRUE
  countFile: 'feature_count/TE_included.txt' # all three formats need to be tested in development
...
outputDir <- paste(getwd(),params$outputDir,sep="/")
outputDeseq <- paste(params$outputDir,"DESeq2",sep="/")
outputDeseqRnk <- paste(params$outputDir,"DESeq2/rnk",sep="/")

dir.create(outputDir, showWarnings = FALSE)
dir.create(outputDeseq, showWarnings = FALSE)
dir.create(outputDeseqRnk, showWarnings = FALSE)
...
paste("DESeq2 outputs:", outputDeseq)
paste("DESeq2 rnk files:",outputDeseqRnk)
```
...
volcanoplot <- function(res, anno, name='name', 
  ...
  # save PDF
  pdf(paste(outputDeseq,paste(name, "pdf", sep="."),sep="/"), width=8, height=6)
  grid.arrange(p,
               ncol=1,
               top = textGrob(' ',
                              just = c('center'),
...
process_deseq_res <- function(res="lfcshrink.res", res2="results.res", name='name', anno='anno.df', norm_exp="tpm.df"){
    ...
    head(resdata.sig)
    print(paste(sep="/",outputDeseq, paste(name, 'deseq2.xlsx', sep = '.' )))

    ## Write results
    WriteXLS(x = resdata,
             ExcelFileName = paste(sep="/",outputDeseq, paste(name, 'deseq2.xlsx', sep = '.')),
             row.names = F, SheetNames = 'sheet1', na = 'NA')  # for user

    WriteXLS(x = resdata.sig,
             ExcelFileName = paste(sep="/",outputDeseq, paste(name, 'deseq2.sig.FDR', max_fdr,
    ...
    rnk[, 1] <- toupper(rnk[, 1])
    write.table(rnk, 
                paste(sep="/",outputDeseq, paste('rnk/', name, '.rnk', sep = '')), 
    ...
    Heatmap(zscore.df, nclass = 2,
            fname = paste(outputDeseq,paste(name, "heatmap", sep="."),sep="/"),
...
head(count_out)
ExcelFileName = paste(outputDir,'COUNT.xlsx',sep="/")
WriteXLS(x = count_out, 
         ExcelFileName = ExcelFileName, row.names = F, SheetNames = 'sheet1', na = 'NA')
print(paste("saved in",ExcelFileName))
```
...
if (featureCountsFormat ){
  ...
  # head(tpm_out)
  # tail(tpm_out)
  ExcelFileName = paste(outputDir,'TPM.xlsx',sep="/")
  WriteXLS(x = tpm_out, 
           ExcelFileName = ExcelFileName, row.names = F, SheetNames = 'sheet1', na = 'NA')
  print(paste("saved in",ExcelFileName))
} else {
...
if (featureCountsFormat ){
  ...
  # head(fpkm_out)
  # tail(fpkm_out)
  ExcelFileName = paste(outputDir,'FPKM.xlsx',sep="/")
  WriteXLS(x = fpkm_out, 
           ExcelFileName = ExcelFileName, row.names = F, SheetNames = 'sheet1', na = 'NA')
  print(paste("saved in",ExcelFileName))
...
ExcelFileName = paste(outputDir,'DESeq2NormalizedCounts.xlsx',sep="/")
...
pdf(paste(outputDir,"sample_PCA.labeled.pdf",sep="/"))
plotQC_PCA(dds)
dev.off()

pdf(paste(sep="/",outputDir,"sample_PCA.pdf"))
...
pheatmap(samplePoisDistMatrix,
         ...
         legend = T, 
         filename = paste(sep="/",outputDir,"sample_poisson_distance.pdf"))
...
## Log
```{r log}
save.image(file=paste(outputDeseq,"RSession"))
sessionInfo()
```
